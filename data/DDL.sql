  ...
  CREATE TABLE FINE_PAID 
   (	ID VARCHAR2(255 BYTE) NOT NULL ENABLE, 
	...
	ASSETCHECKOUT_ID NUMBER(19,0), 
	 PRIMARY KEY (ID), 
	 CONSTRAINT FK_FINE_PAID_PATRON_PATRON_ID FOREIGN KEY (PATRON_PATRON_ID)
	  REFERENCES PATRON (PATRON_ID) ENABLE, 
	 CONSTRAINT FK_FINE_PAID_ASSETCHECKOUT_ID FOREIGN KEY (ASSETCHECKOUT_ID)
	  REFERENCES ASSET_CHECKOUT (ID) ENABLE
   );
...
CREATE OR REPLACE TRIGGER RES_BOOK_USER
BEFORE INSERT OR UPDATE
ON ASSET_CHECKOUT
REFERENCING NEW AS new OLD AS old
FOR EACH ROW
...
show errors TRIGGER RES_BOOK_USER;
