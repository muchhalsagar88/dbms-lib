...
//			checkout();
	...
	public void displayBooks() {
		...
		Object[][] bks = new Object[books.size()][]; 
		int count = 0;
		for(Book b: books)
			bks[count++] = b.toObjectArray();
...
protected List<Book> getBookList() {
		...
		String query = "SELECT b FROM "+Book.class.getName()+" b";
//				+" WHERE b.id NOT IN"
//				+"(SELECT a.asset.id FROM AssetCheckout a WHERE a.dueDate IS NULL)";
		
		return (List<Book>)DBUtils.fetchAllEntities(query);
	}
